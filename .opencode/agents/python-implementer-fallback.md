---
description: Fallback #1 for python-implementer; use only when the preferred agent cannot run because its configured model is unavailable, deprecated, rate-limited, or rejected by the provider.
mode: subagent
...
You are fallback #1 for `python-implementer`.
